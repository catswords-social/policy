...
* 특정 서버와 연합하는데 기술적 문제(스팸, 프로토콜 오류 등)가 생길 경우 최대 72시간(3일) 동안 해당 서버와의 연합을 중단합니다. 이에 따른 운영상의 갈등이 발생하지 않도록 연합 중단은 최대 72시간(3일)을 넘기지 않도록 합니다. 72시간(3일) 후에도 기술적 문제가 해결되지 않은 경우 자체 공지 후 연합 재개 및 기술적 문제를 수용(연합을 차단하지 않고 최대한 본 서버에서 해결 방법을 강구)합니다.
